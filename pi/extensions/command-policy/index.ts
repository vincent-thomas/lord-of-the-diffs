 ...
 *
 * Allows only approved shell commands in the bash tool. The allow-list
 * matching itself lives in the pure @vt-pi/command-policy library; this file
 * owns the only Pi-touching part — wiring that library's matching functions
 * into the bash tool_call hook.
 */

import type { ExtensionAPI } from "@mariozechner/pi-coding-agent";
import { isToolCallEventType } from "@mariozechner/pi-coding-agent";
import { CommandPolicyStatus, getCommandUses, matchesEntry, findBannedFlag, findDisallowedFlag } from "@vt-pi/command-policy";
