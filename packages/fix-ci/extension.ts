/**
 * extension.ts — builds a Pi extension exposing the `push_and_check_ci`
 * tool: pushes code, creates a draft PR if none exists, polls GitHub checks
 * until they finish, and — once green — marks the PR ready and waits for
 * human review, returning failures (or review comments) for the agent to
 * fix and retry. Tracks fix cycles and tells the agent to stop after
 * MAX_CYCLES attempts.
 *
 * Manual `git push` in bash is expected to be blocked by the caller's own
 * command-policy configuration (e.g. @vt-pi/command-policy banning the
 * "git push" subcommand), not here.
 */
